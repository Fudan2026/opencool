...
import { fetchDailyHot } from "./dailyhot";
...
export async function fetchSource(source: SourceDef): Promise<RawArticle[]> {
  ...
  // DailyHot-compatible social hot ranks (douyin / weibo / zhihu / baidu …)
  if (source.type === "api" && source.subcategory === "hot") {
    return fetchDailyHot(source.id, source.url);
  }
